...
class Test_Cart(unittest.TestCase):
    ...
    def test_products_to_cart(self):
        ...
        products_in_cart = []
        products_index_list = []
        for i in range(0, products_quantity_in_cart-1):
            ...
            products_in_cart.append([
                product_index,
                self.inventory_page.get_inventory_item_title_by_index(product_index),
                self.inventory_page.get_inventory_item_price_by_index(product_index)
            ])
            # add product to cart
